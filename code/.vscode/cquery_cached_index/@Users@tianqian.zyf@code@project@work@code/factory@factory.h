...
public:
  ...
  static void registerFactory(Base& factory) {
    ...
    if (!result.second) {
      throw std::runtime_error::runtime_error(absl::StrFormat("Double registration for name: '%s'", factory.name()));
    }
  }
...
private:
  ...
  static Base* replaceFactoryForTest(Base& factory) {
    ...
    ABSL_ASSERT(getFactory(factory.name()) == &factory);
    return displaced;
  }
...
#define REGISTER_FACTORY(FACTORY, BASE)                                                     \
  ...
  static Registry::RegisterFactory</* NOLINT(fuchsia-statically-constructed-objects) */     \
                                          FACTORY, BASE>                                    \
      FACTORY##_registered
